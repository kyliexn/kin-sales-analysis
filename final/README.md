...
| File | What it is |
|---|---|
| `cafe_yoy_report.md` | Final report — read this first |
| `charts/` | All 4 charts referenced in the report |
| `01_load_clean.py` → `05_charts.py` | The analysis pipeline, in order — commented so you can read through the logic |
...
The cafe's menu isn't static — Mochi Donut and Brioche Donut flavors rotate, and GF Muffins
rotate daily. Comparing one flavor's 2024 sales to its 2025 sales mostly reflects how many
weeks/days it happened to be offered, not real demand change. `03_classify_menu.py` splits
items into "permanent" (fair to compare individually) and "rotating" (only fair to compare as
a category total) before any YoY math happens — that distinction is what the whole analysis
is built on.
